...
function renderBillingStudentsTable(filterText = '') {
    ...
    filtered.forEach(inv => {
        ...
        tr.innerHTML = `
            ...
            <td class="text-right">
                <button class="btn btn-outline" style="border-color: #EF4444; color: #EF4444;" title="Return Items" onclick="openInvoiceReturnModal('${inv.id}', event)">
                    <i class="ph ph-arrow-counter-clockwise"></i> Return Items
                </button>
            </td>
        `;
        tbody.appendChild(tr);
...
function openInvoiceReturnModal(invoiceId, evt) {
    if (evt) {
        evt.preventDefault();
        evt.stopPropagation();
    }
    ...
    if (!invoiceId) return;
    activeReturnInvoiceId = String(invoiceId);
    fetchAndRenderReturnInvoice(activeReturnInvoiceId);
}

function fetchAndRenderReturnInvoice(invoiceId) {
    if (!invoiceId) return;
    ...
    fetch(`${API_BASE}/invoices/${invoiceId}/details`)
        ...
        .then(({ status, data }) => {
            ...
            renderReturnItemsModal(invoice, items, student, schoolName);
        })
        .catch((err) => {
            console.error('fetchAndRenderReturnInvoice error', err);
            showToast('Connection error while loading return details', 'error');
        });
}

function renderReturnItemsModal(invoice, items, student, schoolName) {
    const invoiceLabel = String(invoice?.invoice_number || `INV-${invoice?.id || 'NA'}`);
    const studentName = student?.student_name || student?.name || '-';

    const invoiceEl = document.getElementById('return-invoice-label');
    const studentEl = document.getElementById('return-student-name');
    const tbody = document.getElementById('return-items-body');
    const totalEl = document.getElementById('return-items-total-value');

    if (invoiceEl) invoiceEl.textContent = invoiceLabel;
    if (studentEl) studentEl.textContent = `${studentName} (${schoolName || '-'})`;
    if (!tbody) return;
    ...
    items.forEach((line) => {
        ...
        tr.innerHTML = `
            ...
            <td class="text-right">
                <button class="btn btn-outline" style="border-color: #EF4444; color: #EF4444;" onclick="returnInvoiceItem('${line?.id || ''}', event)">
                    <i class="ph ph-arrow-counter-clockwise"></i> Return
                </button>
            </td>
        `;
        tbody.appendChild(tr);
    ...
    if (!items.length) {
        const tr = document.createElement('tr');
        tr.innerHTML = '<td colspan="4" class="text-center text-muted">No purchased items found for this invoice.</td>';
        tbody.appendChild(tr);
    }
...
function returnInvoiceItem(invoiceItemId, evt) {
    if (evt) {
        evt.preventDefault();
        evt.stopPropagation();
    }

    const invoiceId = String(activeReturnInvoiceId || '');
    if (!invoiceId || !invoiceItemId) return;

    const isConfirmed = window.confirm('Return this item and add it back to inventory?');
    if (!isConfirmed) return;

    fetch(`${API_BASE}/invoices/${invoiceId}/return-item`, {
        method: 'POST',
        ...
        .then(({ status, data }) => {
            ...
            const restoredQty = Number(data?.restored_quantity || 0);

            if (data?.invoice_deleted) {
                closeReturnItemsModal();
                activeReturnInvoiceId = null;
                showToast(restoredQty > 0
                    ? `Item returned. Invoice deleted automatically. (+${restoredQty} stock)`
                    : 'Item returned. Invoice deleted automatically.');
                fetchInvoices({ force: true });
                loadStocks();
                return;
            }
            ...
            showToast(restoredQty > 0
                ? `Item returned successfully. (+${restoredQty} stock)`
                : 'Item returned successfully.');

            fetchInvoices({ force: true });
            loadStocks();
            fetchAndRenderReturnInvoice(invoiceId);
        })
        .catch((err) => {
            console.error('returnInvoiceItem error', err);
...
function renderPaperBillFromSavedInvoice(invoice, items, student, schoolName, options = {}) {
    const returnMode = Boolean(options?.returnMode);
    const billModal = document.getElementById('bill-draft-modal');
    if (billModal) {
        billModal.classList.toggle('return-list-mode', returnMode);
    }

    const printBtn = document.querySelector('#bill-draft-modal .paper-bill-actions .btn-primary');
    if (printBtn) {
        printBtn.classList.toggle('hidden', returnMode);
    }
    ...
    const headerRow = document.querySelector('#bill-draft-modal .paper-bill-items thead tr');
    if (headerRow) {
        if (returnMode) {
            headerRow.innerHTML = `
    ...
    items.forEach((line, index) => {
        ...
        if (returnMode) {
            tr.innerHTML = `
                ...
                <td style="text-align:center"><button class="btn btn-outline" style="border-color: #EF4444; color: #EF4444;" onclick="returnInvoiceItem('${line?.id || ''}', event)"><i class="ph ph-arrow-counter-clockwise"></i> Return</button></td>
            `;
        } else {
            tr.innerHTML = `
...
function renderInvoicesTable() {
    ...
    draftInvoices.forEach(inv => {
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge ${statusCls}">${status}</span></td>
            <td class="text-right">
                <button class="btn btn-outline" style="border-color: #EF4444; color: #EF4444;" title="Delete Invoice" onclick="deleteInvoice('${inv.id}', event)">
...
function deleteInvoice(invoiceId, evt) {
    if (evt) {
        evt.preventDefault();
        evt.stopPropagation();
    }
    ...
    const invoice = state.invoices.find(inv => String(inv.id || '') === String(invoiceId));
    const invoiceLabel = String(invoice?.invoice_number || `INV-${invoiceId}`);
    const status = String(invoice?.status || '').toLowerCase();
    const paidHint = status === 'paid'
        ? '\n\nPurchased quantities will be added back to inventory.'
        : '';

    const isConfirmed = window.confirm(`Delete invoice ${invoiceLabel}?${paidHint}`);
    if (!isConfirmed) return;
    ...
    fetch(`${API_BASE}/invoices/${invoiceId}`, {
        ...
        .then(({ status, data }) => {
            ...
            state.invoices = state.invoices.filter(inv => String(inv.id || '') !== String(invoiceId));
            renderInvoicesTable();
            renderBillingStudentsTable(document.getElementById('search-billing-student')?.value || '');
            loadStocks();
            ...
            const restoredQty = Number(data?.restored_quantity || 0);
            if (restoredQty > 0) {
                showToast(`Invoice deleted. ${restoredQty} item(s) added back to inventory.`);
            } else {
        ...
        .catch((err) => {
            ...
            showToast('Connection error while deleting invoice', 'error');
        });
}
...
function showPaperBill(invoiceId, status) {
    ...
    const billModal = document.getElementById('bill-draft-modal');
    if (billModal) {
        billModal.classList.remove('return-list-mode');
    }
